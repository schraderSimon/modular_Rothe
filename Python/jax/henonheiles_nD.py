...
rothesolver=RotheSolver(SHH2, dt, 0, p_init, c_init, rothe_grad_jax=rothe_vg_jit,
                        rothe_nograd=rothe_error, splitting_type=splitting_type,
                        name="HenonHeiles_%dD_%d"%(D,n), polynomial_string=example_string)
nsteps=int(100/dt)
try:
    ...
    save_rothe_state(rothesolver.name, splitting_type, example_string, dt,
                     0.0, float(initial_error), p_init, c_init)
    print("No prior file. Starting fresh at t=0.")
    
rothesolver.propagate(nsteps, maxiter=500)